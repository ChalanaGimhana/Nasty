...
export default class SetupButtons extends Event {
    ...
    public async run(interaction: any): Promise<void> {
        ...
        if (!interaction.member.voice.channel) return await buttonReply(interaction, `You are not connected to a voice channel to use this button.`, this.client.color.red);
        ...
        if (message) {
            switch (interaction.customId) {
                case 'LOW_VOL_BUT':
                    ...
                    await message.edit({ embeds: [embed.setFooter({ text: `Volume: ${vol.toFixed()}%`, iconURL: interaction.member.displayAvatarURL({ }) })] });
                    break;
                case 'HIGH_VOL_BUT':
                    ...
                    await message.edit({ embeds: [embed.setFooter({ text: `Volume: ${vol2.toFixed()}%`, iconURL: interaction.member.displayAvatarURL({ }) })] });
                    break;
                case 'PAUSE_BUT':
                    const name = player.player.paused ? `Resumed` : `Paused`;
                    player.pause();
                    await buttonReply(interaction, `${name} the music.`, this.client.color.main);
                    await message.edit({ embeds: [embed.setFooter({ text: `${name} by ${interaction.member.displayName}`, iconURL: interaction.member.displayAvatarURL({ }) })] });
                    break;
                case 'SKIP_BUT':
                    if (player.queue.length === 0) return await buttonReply(interaction, `There is no music to skip.`, this.client.color.main);
                    player.skip();
                    await buttonReply(interaction, `Skipped the music.`, this.client.color.main);
                    await message.edit({ embeds: [embed.setFooter({ text: `Skipped by ${interaction.member.displayName}`, iconURL: interaction.member.displayAvatarURL({ }) })] });
                    break;
                case 'STOP_BUT':
                    player.stop();
                    await buttonReply(interaction, `Stopped the music.`, this.client.color.main);
                    await message.edit({ embeds: [embed.setFooter({ text: `Stopped by ${interaction.member.displayName}`, iconURL: interaction.member.displayAvatarURL({ }) })] });
                    break;
                case 'LOOP_BUT':
                    const random = ["off", "queue", "repeat"];
                    const loop = random[Math.floor(Math.random() * random.length)];
                    if (player.loop === loop) return await buttonReply(interaction, `Loop is already ${player.loop}.`, this.client.color.main);
                    player.setLoop(loop);
                ...
                case 'SHUFFLE_BUT':
                    ...
                    await message.edit({ embeds: [embed.setFooter({ text: `Shuffle set to ${player.shuffle ? `on` : `off`} by ${interaction.member.displayName}`, iconURL: interaction.member.displayAvatarURL({ }) })] });
                    break;
                case 'PREV_BUT':
                    ...
                    await message.edit({ embeds: [embed.setFooter({ text: `Playing the previous track by ${interaction.member.displayName}`, iconURL: interaction.member.displayAvatarURL({ }) })] });
                    break;
                case 'REWIND_BUT':
                    const time = player.player.position - 10000;
                    if (time < 0) return await buttonReply(interaction, `You cannot rewind the music more than the length of the song.`, this.client.color.main);
                    player.seek(time);
                    await buttonReply(interaction, `Rewinded the music.`, this.client.color.main);
                    await message.edit({ embeds: [embed.setFooter({ text: `Rewinded by ${interaction.member.displayName}`, iconURL: interaction.member.displayAvatarURL({ }) })] });
                    break;
                case 'FORWARD_BUT':
                    const time2 = player.player.position + 10000;
                    if (time2 > player.current.info.length) return await buttonReply(interaction, `You cannot forward the music more than the length of the song.`, this.client.color.main);
                    player.seek(time2);
                    await buttonReply(interaction, `Forwarded the music.`, this.client.color.main);
                    await message.edit({ embeds: [embed.setFooter({ text: `Forwarded by ${interaction.member.displayName}`, iconURL: interaction.member.displayAvatarURL({ }) })] });
                    break;
                default:
