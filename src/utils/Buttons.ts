...
function getButtons(): ActionRowBuilder<ButtonBuilder>[] {
    ...
    let rewindbut = new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(false);
    let row = new ActionRowBuilder<ButtonBuilder>().addComponents(lowvolumebut, previousbut, pausebut, skipbut, highvolumebut);
    let row2 = new ActionRowBuilder<ButtonBuilder>().addComponents(rewindbut, loopbut, stopbut, shufflebut, forwardbut);
    return [row, row2];
}
